...
terraform { // This is to Store Remote State
  backend "s3" {
    bucket = "ryan5100"   // Bucket where to SAVE Terraform State
    key    = "dev/webserver/terraform.tfstate" // Object name in the bucket to SAVE Terraform State
    region = "ap-northeast-2"                      // Region where bucket created
  }
}
...
data "terraform_remote_state" "vpc" { // This is to use Outputs from Remote State
  backend = "s3"
  config = {
    bucket = "ryan5100" // Bucket from where to GET Terraform State
...
resource "aws_instance" "web_server" {
  ...
  subnet_id              = data.terraform_remote_state.vpc.outputs.public_subnet_ids[0] //output <가져옴
  user_data              = <<EOF
#!/bin/bash
yum -y update
yum -y install httpd
...
resource "aws_security_group" "webserver" {
  ...
  vpc_id = data.terraform_remote_state.vpc.outputs.vpc_id //output <가져옴
  ...
  ingress {
    ...
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = [data.terraform_remote_state.vpc.outputs.vpc_cidr] //output <가져옴
  }
  egress {
    from_port   = 0
    to_port     = 0
